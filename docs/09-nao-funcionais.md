...
| RNF-019 | Backup com restauração **testada uma vez, para valer**, a partir do OBJETO NO BUCKET — nunca da cópia local | Exercitado em 27/08/2026 |
| RNF-021 | O dump do Postgres **DEVE** ter cópia fora da máquina, pelo `enviar-r2.sh` da VPS. Falha no envio não derruba o backup local, mas sai com erro para o alerta | Exercitado na VPS em 27/08/2026 |
| RNF-020 | Na subida, o depósito de avatares é **sondado** (gravar, ler, conferir, apagar) e o resultado vai para o log. Não é fatal: jogar não depende de foto (I-1) | Teste unitário (RNF-020) |
