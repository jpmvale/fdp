 ...
 */
export const DIFICULDADES = ['FACIL', 'MEDIO', 'DIFICIL', 'REALISTA'];
/** As que hoje têm comportamento próprio. */
export const DIFICULDADES_PRONTAS = ['FACIL', 'MEDIO'];
...
const VALORES = 13;
const escolher = (itens, rng) => itens[rng.nextInt(itens.length)] ?? itens[0];
/**
 ...
 */
function apostasPermitidas(visao) {
    const todas = [];
    for (let v = 0; v <= visao.cardsThisRound; v++) {
        if (v !== visao.forbiddenBet)
            todas.push(v);
    }
    return todas;
}
...
// Aposta
// ---------------------------------------------------------------------------
export function decidirAposta(visao, dificuldade, rng) {
    ...
    if (dificuldade === 'FACIL')
        return escolher(permitidas, rng);
...
// Carta
// ---------------------------------------------------------------------------
export function decidirCarta(visao, dificuldade, rng) {
    ...
    if (mao.length === 0)
        throw new Error('decidirCarta sem cartas na mão');
    if (dificuldade === 'FACIL')
        return escolher(mao, rng).id;
    ...
    if (perdedoras.length > 0)
        return perdedoras[perdedoras.length - 1].id;
