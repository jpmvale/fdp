...
import { absentMatchPlayers, deadlineFor, dealNow, sealMatchEnd, seatedPlayers, translate, } from './room.js';
import { isAbsent, isOnline, isPresent, } from './types.js';
...
export function nextDeadline(room) {
    ...
    if (room.pause) {
        if (!room.pause.decisionAnnounced)
            candidates.push(room.pause.decisionUnlockedAt);
...
export function tick(room, ctx) {
    ...
    let changed = false;
    const step = (result) => {
        if (result.room !== current || result.emissions.length > 0)
            changed = true;
        current = result.room;
 ...
 * ponto.
 */
function expireTransportGrace(room, ctx) {
    const emissions = [];
    ...
    for (const player of room.players) {
        if (player.connection !== 'RECONECTANDO' || player.socketLostAt === null)
            continue;
        if (ctx.now < player.socketLostAt + LIMITS.transportGraceMs)
            continue;
        anyExpired = true;
        players = players.map((p) => p.id === player.id ? { ...p, connection: 'DESCONECTADO' } : p);
        emissions.push(all({
...
function enterPause(room, ctx, emissions) {
    const pause = {
        since: ctx.now,
...
function ensureHost(room, emissions) {
...
function advancePause(room, ctx) {
 ...
 */
function advanceMatchClock(room, ctx) {
    if (room.status !== 'EM_PARTIDA' || !room.match)
        return { room, emissions: [] };
    if (room.match.endReason !== null)
        return { room, emissions: [] };
 ...
 */
function playBot(room, difficulty, botId, ctx) {
    if (!room.match)
        return { room, emissions: [] };
