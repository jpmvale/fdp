...
import { isOnline, isPresent, type Room } from './types.js';

export function checkRoomInvariants(room: Room): string[] {
  ...
  if (room.status === 'PAUSADA') {
    ...
    if (room.pause === null) violations.push('INV-14: status PAUSADA sem estado de pausa');
  }
  if (room.status !== 'PAUSADA' && room.pause !== null) {
    violations.push('INV-14: estado de pausa presente fora de PAUSADA');
  }

  if (room.match) violations.push(...checkMatchInvariants(room.match));
  return violations;
}
