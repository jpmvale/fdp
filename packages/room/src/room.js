 ...
 */
import { AVATAR_COLORS, AVATAR_EMOJIS, LIMITS, } from '@fdp/protocol';
import { activePlayers, advance, applyMove, createMatch, DEFAULT_OPTIONS, endMatch, isActive, project, withdrawPlayers, } from '@fdp/rules';
import { isAbsent, isOnline, isPresent, toPublicPlayer, } from './types.js';
function failWith(code, motivo) {
    return { ok: false, code, motivo };
}
/** Toda mudança de estado passa por aqui: `stateVersion` nunca é esquecido. */
function commit(room, ctx, emissions) {
 ...
 */
export function sealMatchEnd(room, emissions) {
    if (room.status !== 'EM_PARTIDA')
        return room;
    if (!room.match || room.match.endReason === null)
        return room;
...
const all = (event) => ({ audience: 'ALL', event });
const to = (playerId, event) => ({
    audience: { playerId },
    event,
});
// ---------------------------------------------------------------------------
// Criação e entrada
// ---------------------------------------------------------------------------
export function createRoom(code, host, ctx) {
    return {
        code,
        status: 'LOBBY',
...
function newPlayer(params, now, isSpectator) {
    return {
        id: params.playerId,
...
function newBot(room, difficulty, now, ctx) {
    ...
    const tomados = new Set(room.players.filter(isPresent).map((p) => `${p.avatar.emoji}|${p.avatar.color}`));
    const avatar = AVATARES.find((a) => !tomados.has(`${a.emoji}|${a.color}`)) ?? AVATARES[0];
    ...
    let id = ctx.newId();
    for (let n = 2; existentes.has(id); n++)
        id = `${ctx.newId()}-${n}`;
    return {
        id,
        nickname,
...
export const botsOf = (room) => room.players.filter((p) => isPresent(p) && p.bot !== null);
export function seatedPlayers(room) {
    return room.players.filter((p) => isPresent(p) && !p.isSpectator);
}
export function spectators(room) {
    return room.players.filter((p) => isPresent(p) && p.isSpectator);
}
export function join(room, params, ctx) {
    if (room.status === 'ENCERRADA')
        return failWith('WRONG_STATUS', 'SALA_ENCERRADA');
    const existing = room.players.find((p) => p.id === params.playerId);
    if (existing)
        return reconnect(room, params.playerId, ctx);
 ...
 * retomar a partida.
 */
export function reconnect(room, playerId, ctx) {
    const player = room.players.find((p) => p.id === playerId);
    if (!player)
        return failWith('VALIDATION_FAILED', 'JOGADOR_DESCONHECIDO');
    if (!isPresent(player))
        return failWith('WRONG_STATUS', 'JOGADOR_SAIU');
 ...
 */
export function disconnect(room, playerId, ctx) {
    const player = room.players.find((p) => p.id === playerId);
    if (!player || !isPresent(player))
        return failWith('VALIDATION_FAILED', 'JOGADOR_DESCONHECIDO');
    if (!isOnline(player))
        return { ok: true, room, emissions: [] };
    const players = replace(room.players, playerId, {
...
export function leave(room, playerId, ctx) {
    const player = room.players.find((p) => p.id === playerId);
    if (!player || !isPresent(player))
        return failWith('VALIDATION_FAILED', 'JOGADOR_DESCONHECIDO');
    let next = {
 ...
 */
function succeedHost(room, emissions) {
    const host = room.players.find((p) => p.id === room.hostId);
    if (host && isPresent(host) && isOnline(host))
        return room;
    ...
    if (!successor)
        return room; // ninguém online; sucede na próxima conexão
    if (successor.id === room.hostId)
        return room;
    emissions.push(all({ type: 'room:hostChanged', payload: { hostId: successor.id } }));
    return { ...room, hostId: successor.id };
}
function replace(players, playerId, patch) {
    return players.map((p) => (p.id === playerId ? { ...p, ...patch } : p));
}
// ---------------------------------------------------------------------------
// Pausa (`03` §1.2)
// ---------------------------------------------------------------------------
export function absentMatchPlayers(room) {
    if (!room.match)
        return [];
    return room.players
...
export function pauseMatch(room, ctx, emissions) {
    if (room.status !== 'EM_PARTIDA')
        return room;
    const pause = {
        since: ctx.now,
 ...
 */
function maybeResume(room, ctx, emissions) {
    if (room.status !== 'PAUSADA' || !room.match)
...
export function deadlineFor(room, now) {
    if (!room.match || room.match.endReason !== null)
        return null;
...
// Comandos (`05` §4)
// ---------------------------------------------------------------------------
export function applyCommand(room, playerId, command, ctx) {
    const player = room.players.find((p) => p.id === playerId);
    if (!player || !isPresent(player))
        return failWith('VALIDATION_FAILED', 'JOGADOR_DESCONHECIDO');
    ...
    if (hostOnly && !isHost)
        return failWith('NOT_HOST', 'COMANDO_EXIGE_HOST');
    switch (command.type) {
        case 'room:resync':
        ...
        case 'player:setProfile': {
            if (room.status !== 'LOBBY')
                return failWith('WRONG_STATUS', 'SO_NO_LOBBY');
            const players = replace(room.players, playerId, {
        ...
        case 'chat:send': {
            ...
            if (player.bot !== null)
                return failWith('VALIDATION_FAILED', 'BOT_NAO_FALA');
        ...
        case 'host:kick': {
            ...
            if (target === playerId)
                return failWith('VALIDATION_FAILED', 'HOST_NAO_SE_EXPULSA');
            if (!room.players.some((p) => p.id === target && isPresent(p))) {
                return failWith('VALIDATION_FAILED', 'JOGADOR_DESCONHECIDO');
            }
            return commit({ ...room, players: replace(room.players, target, { connection: 'REMOVIDO' }) }, ctx, [all({ type: 'room:playerLeft', payload: { playerId: target, reason: 'KICKED' } })]);
        }
        case 'host:addBot': {
            if (room.status !== 'LOBBY')
                return failWith('WRONG_STATUS', 'SO_NO_LOBBY');
        ...
        case 'host:removeBot': {
            ...
            if (!alvo || alvo.bot === null)
                return failWith('VALIDATION_FAILED', 'NAO_E_BOT');
            return commit({ ...room, players: room.players.filter((p) => p.id !== alvo.id) }, ctx, [all({ type: 'room:playerLeft', payload: { playerId: alvo.id, reason: 'LEFT' } })]);
        }
        ...
        case 'host:rematch': {
            if (room.status !== 'FIM_DE_PARTIDA')
                return failWith('WRONG_STATUS', 'SEM_FIM_DE_PARTIDA');
            // Espectadores viram jogadores na volta ao lobby (RF-014).
            const players = room.players.map((p) => (isPresent(p) ? { ...p, isSpectator: false } : p));
            const lobby = { ...room, status: 'LOBBY', match: null, pause: null, phaseDeadline: null, players };
 ...
 */
export function dealNow(room, ctx, emissions) {
    let current = room;
    let guard = 0;
    while (current.match !== null &&
        ...
        if (!result.ok)
            break;
        const next = { ...current, match: result.state };
        current = { ...next, phaseDeadline: deadlineFor(next, ctx.now) };
        emissions.push(...translate(result.events, current));
    }
    return current;
}
function startMatch(room, ctx) {
    if (room.status !== 'LOBBY')
        return failWith('WRONG_STATUS', 'SO_NO_LOBBY');
    const seated = seatedPlayers(room);
    if (seated.length < LIMITS.minPlayers)
        return failWith('WRONG_STATUS', 'JOGADORES_INSUFICIENTES');
    ...
    const started = dealNow({ ...room, status: 'EM_PARTIDA', match, phaseDeadline: null }, ctx, emissions);
    return commit(started, ctx, emissions);
}
function applyGameMove(room, playerId, command, ctx) {
    if (room.status === 'PAUSADA')
        return failWith('MATCH_PAUSED', 'PARTIDA_PAUSADA');
    if (room.status !== 'EM_PARTIDA' || !room.match)
        return failWith('WRONG_STATUS', 'SEM_PARTIDA');
    if (command.payload.matchId !== room.match.id)
        return failWith('STALE_MOVE', 'PARTIDA_ANTIGA');
    const move = command.type === 'move:bet'
        ? {
            type: 'bet',
            playerId,
    ...
    if (!result.ok)
        return { ok: false, code: result.code, motivo: result.motivo };
    const withMove = {
        ...room,
        match: result.state,
...
function resolveAbsence(room, action, ctx) {
    if (room.status !== 'PAUSADA' || !room.pause)
        return failWith('WRONG_STATUS', 'SEM_PAUSA');
    ...
    if (action === 'ENCERRAR')
        return finishMatch(room, ctx, 'ENCERRADA_POR_AUSENCIA');
    const absent = absentMatchPlayers(room);
    if (absent.length === 0)
        return failWith('WRONG_STATUS', 'NINGUEM_AUSENTE');
    if (!room.match)
        return failWith('WRONG_STATUS', 'SEM_PARTIDA');
    const withdrawal = withdrawPlayers(room.match, absent, ctx);
    if (!withdrawal.ok)
        return failWith('WRONG_STATUS', withdrawal.motivo);
    let next = {
        ...room,
    ...
    const emissions = [
        ...
        ...absent.map((id) => all({ type: 'room:playerLeft', payload: { playerId: id, reason: 'WITHDRAWN' } })),
        all({ type: 'room:statusChanged', payload: { status: 'EM_PARTIDA' } }),
    ];
    if (withdrawal.state.endReason !== null) {
...
function finishMatch(room, ctx, reason) {
 ...
 */
export function translate(events, room) {
...
export function snapshotFor(room, viewerId) {
    return {
        type: 'room:snapshot',
        payload: {
            code: room.code,
