...
export function checkInvariants(state) {
    ...
    if (dealt) {
        ...
        const played = round.resolvedTricks.reduce((n, t) => n + t.plays.length, 0) +
        ...
        for (const h of Object.values(hidden.hands))
            h.forEach(visit);
        hidden.stock.forEach(visit);
        for (const t of round.resolvedTricks)
            t.plays.forEach((p) => visit(p.cardId));
        round.currentTrick?.plays.forEach((p) => visit(p.cardId));
        if (dup.length > 0)
            violations.push(`INV-04: cartas duplicadas ${dup.join(', ')}`);
        // INV-18
    ...
    for (const id of state.playerOrder) {
        if (state.lives[id] === undefined)
            violations.push(`INV-06: ${id} sem vidas`);
    }
    ...
    const allBetsIn = round.bidOrder.length > 0 &&
 ...
 */
export function checkNoLeak(state, viewerId) {
    if (Object.keys(state.hidden.cards).length === 0)
        return [];
    const serialized = JSON.stringify(project(state, viewerId));
    const violations = [];
    const foreheadRevealed = state.round.isForeheadRound && state.round.phase === 'RESOLUCAO';
