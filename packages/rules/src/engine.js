...
import { forbiddenBetFor, nextCardsThisRound, nextFirstBidder, orderFrom, } from './round.js';
import { isDoomed, nextLeaderOf, resolveTrick } from './trick.js';
import { DEFAULT_OPTIONS, } from './types.js';
const fail = (code, motivo) => ({ ok: false, code, motivo });
/** Jogador ainda na partida: nem eliminado, nem retirado. */
export function isActive(state, playerId) {
    return (!state.eliminated.some((e) => e.playerId === playerId) &&
...
export function createMatch(params) {
    ...
    for (const id of playerOrder)
        lives[id] = options.vidasIniciais;
    const state = {
        id: params.matchId,
        seed: params.seed,
        options,
...
function validateOptions(options, playerCount) {
...
export function advance(state, ctx) {
    if (state.endReason !== null)
        return fail('WRONG_STATUS', 'MATCH_ENCERRADA');
...
function deal(state) {
    ...
    if (shoe.length < needed) {
        throw new Error(`sabot insuficiente: ${shoe.length} cartas para ${needed} necessárias`);
    }
    const bidOrder = orderFrom(state.playerOrder, state.firstBidderId, (id) => players.includes(id));
...
function reveal(state) {
    ...
    for (const play of plays)
        revealed[play.playerId] = play.cardId;
...
// Jogadas
// ---------------------------------------------------------------------------
export function applyMove(state, move, ctx) {
    if (state.endReason !== null)
        return fail('WRONG_STATUS', 'MATCH_ENCERRADA');
...
function applyBet(state, playerId, bet) {
    ...
    if (isLastBidder) {
        const forbidden = forbiddenBetFor(cardsThisRound, placed.map((id) => round.bets[id]));
    ...
    if (remaining.length > 0) {
        ...
        const nextForbidden = stillToPlace === 1
            ? forbiddenBetFor(cardsThisRound, round.bidOrder.filter((id) => bets[id] !== undefined).map((id) => bets[id]))
            : null;
        events.push({
...
function openTrick(state, leaderId) {
    const playOrder = orderFrom(state.playerOrder, leaderId, (id) => state.round.bidOrder.includes(id));
    return {
        leaderId,
        playOrder,
        plays: [],
 ...
 */
function settleTrick(state, trick) {
    const { round } = state;
    ...
    for (const playerId of round.bidOrder) {
        if (mortoEmVaza[playerId] != null)
            continue;
        const doomed = isDoomed(round.bets[playerId], tricksWon[playerId] ?? 0, tricksRemaining, state.lives[playerId]);
        if (doomed) {
...
function resolveRound(state) {
    ...
    const events = [{ type: 'round:resolved', summary }];
...
function startNextRound(state) {
    ...
    const cardsThisRound = nextCardsThisRound(state.cardsThisRound, state.options.maxCartasPorRodada);
    return {
        ...state,
        roundNumber: state.roundNumber + 1,
        cardsThisRound,
        deckCount: deckCountFor(survivors.length, cardsThisRound),
        firstBidderId: nextFirstBidder(state.playerOrder, state.firstBidderId, (id) => survivors.includes(id)),
        round: emptyRound(),
        hidden: { stock: [], hands: {}, cards: {} },
    };
}
function finish(state, winnerIds, endReason, events) {
    return {
        ok: true,
 ...
 */
export function withdrawPlayers(state, playerIds, ctx) {
    if (state.endReason !== null)
        return fail('WRONG_STATUS', 'MATCH_ENCERRADA');
    const targets = playerIds.filter((id) => isActive(state, id));
    if (targets.length === 0)
        return fail('VALIDATION_FAILED', 'JOGADOR_INATIVO');
...
export function endMatch(state, endReason) {
    if (state.endReason !== null)
        return state;
    return {
        ...state,
