 ...
 */
export function autoCard(state, playerId) {
    const hand = state.hidden.hands[playerId] ?? [];
    if (hand.length === 0)
        throw new Error(`mão vazia para ${playerId}`);
...
export function autoMove(state) {
    ...
    if (playerId === null)
        throw new Error('não há jogador na vez para auto-play');
    if (!isActive(state, playerId))
        throw new Error(`${playerId} não está ativo`);
    const base = {
        playerId,
